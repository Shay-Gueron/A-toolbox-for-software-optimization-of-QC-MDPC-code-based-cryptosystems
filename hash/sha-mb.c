...
* AVX2/AVX512 instructions set" by S. Gueron and R. Shemy at
                           ...
                           IN const uint32_t len)
{
    ...
    {
        digest[i].qwords[0] = bswap_64(ctx->A[i]);
        digest[i].qwords[1] = bswap_64(ctx->B[i]);
        digest[i].qwords[2] = bswap_64(ctx->C[i]);
        digest[i].qwords[3] = bswap_64(ctx->D[i]);
        digest[i].qwords[4] = bswap_64(ctx->E[i]);
        digest[i].qwords[5] = bswap_64(ctx->F[i]);
#ifndef SHA384
        digest[i].qwords[6] = bswap_64(ctx->G[i]);
        digest[i].qwords[7] = bswap_64(ctx->H[i]);
#endif
    }
}
