...
_INLINE_ uint32_t sha_update(sha512_hash_t* hash, const uint64_t *msg, uint32_t n)
{
    ...
    while(n>0)
    {
        a = hash->qwords[0]; b = hash->qwords[1];
        c = hash->qwords[2]; d = hash->qwords[3];
        e = hash->qwords[4]; f = hash->qwords[5];
        g = hash->qwords[6]; h = hash->qwords[7];
        ...
        hash->qwords[0] += a; hash->qwords[1] += b;
        hash->qwords[2] += c; hash->qwords[3] += d;
        hash->qwords[4] += e; hash->qwords[5] += f;
        hash->qwords[6] += g; hash->qwords[7] += h;
        ...
        IN const uint8_t *msg)
{
    ...
    sha512_hash_t hash = {.qwords = INIT_HASH};
    ...
    {
        hash_out->qwords[i] = bswap_64(hash.qwords[i]);
    }
#endif 
